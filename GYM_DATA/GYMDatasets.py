...
def parallel_skeleton(ins_data):
    # print('shape',ins_data.shape)
    # right_shoulder = ins_data[:, :, 8]  # 9th joint
    # left_shoulder = ins_data[:, :, 4]  # 5tf joint

    # Coco format
    right_shoulder = ins_data[:, :, 6]
    left_shoulder = ins_data[:, :, 5]
    #
    ...
    # print(y_tms)
    new_skel = np.zeros(shape=(0, 17, 2))
    ...
    for ind, each_s in enumerate(ins_data):
        # print(each_s.shape)
        r = np.reshape(each_s, newshape=(17, 2))
        r = np.transpose(r)
        r = np.dot(y_tms[ind], r)
        r_t = np.transpose(r)
        r_t = np.reshape(r_t, newshape=(1, -1, 2))
...
class GYMMotionProcessor(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.data_type == 'relative':
            ...
            # Coco format 
            # center = 5 + 6 // 2
            center = (data_numpy[:, :, 5, :].reshape([C, T, 1, M]) + data_numpy[:, :, 6, :].reshape([C, T, 1, M])) / 2
            data_numpy = data_numpy - center
            #
    ...
    def real_resize(data_numpy, length, crop_size):
        ...
        # print(C,T,V,M,data_numpy.shape)
        for i in range(M):
